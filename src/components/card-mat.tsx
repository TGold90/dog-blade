...
function CardMat({ deckCount, incomingCards, isOpponent = false, onDeckClick }: CardMatProps) {
  ...
  return (
    <div className={`relative bg-linear-to-br ${isOpponent ? 'from-red-900/20 to-red-700/20' : 'from-green-900/20 to-green-700/20'} border-4 ${isOpponent ? 'border-red-500' : 'border-green-500'} rounded-xl p-4 min-h-[180px] group`}>
      {/* Tooltip */}
      ...
      <div className={`absolute ${deckPosition === 'right' ? 'right-4' : 'left-4'} top-1/2 transform -translate-y-1/2`}>
        <Deck 
